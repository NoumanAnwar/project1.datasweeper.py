...
if uploaded_files:
    for file in uploaded_files:
        ...
        if file_ext == ".csv":
            df = pd.read_csv(file)
        elif file_ext == ".xlsx":  # ✅ Fixed missing dot
            df = pd.read_excel(file, engine='openpyxl')  # ✅ Specify engine
        else:
        ...
        if st.button(f"Convert {file.name}"):
            ...
            if conversion_type == "CSV":
                df.to_csv(buffer, index=False)  # ✅ Corrected method
                file_name = file.name.replace(file_ext, ".csv")
                mime_type = "text/csv"

            elif conversion_type == "Excel":
                df.to_excel(buffer, index=False, engine='openpyxl')  # ✅ Corrected method
